 ...
 * @LastEditTime: 2025-12-18 15:56:01
...
const ORIGINAL_STYLE = 'ORIGINAL_STYLE_OGE'
class GeomEditor extends BaseObject implements GeomEditorI {
  ...
  #onSelectedChange() {
    const add = (e: CollectionEvent<Feature<Geometry>>) => {
      ...
      feature.set(ORIGINAL_STYLE, original)
      // 后设置选中样式
    ...
    const remove = (e: CollectionEvent<Feature<Geometry>>) => {
      ...
      const feature = e.element
      // 先恢复原样式
      const original = feature.get(ORIGINAL_STYLE)
  ...
  #createTranslateEvent(features: Collection<Feature<Geometry>>, startAt: Coordinate, endAt?: Coordinate) {
    ...
    if (endAt) {
      ...
      return new GeomEditorTranslateEvent(
        GeomEditorEventType.TRANSLATE_END,
        dataList,
        features,
        _startAt,
        _endAt,
    ...
    return new GeomEditorTranslateEvent(GeomEditorEventType.TRANSLATE_START, dataList, features, _startAt)
  }
